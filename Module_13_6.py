...
# Создать клавиатуру InlineKeyboardMarkup с 2 кнопками InlineKeyboardButton:
# 1. С текстом 'Рассчитать норму калорий' и callback_data='calories'.
# 2. С текстом 'Формулы расчёта' и callback_data='formulas'.
# Создать новую функцию main_menu(message), которая:
...
# Создать новую функцию get_formulas(call), которая:
...
# Изменить функцию set_age и декоратор для неё:
# 1. Декоратор сменить на callback_query_handler, который будет реагировать на текст 'calories'.
